    ...
    <table border="2px" style="width: 100%;
        ...
        <tbody>
            @php
                $no=1;
            @endphp
            @foreach ($dataListDesa as $lp)
            <tr>
                <td>{{ $no }}</td>
                <td>{{ $lp->nama }}</td>
                <td>@php
                    $keterangan = '';
                    if ($dataAkunDesa->contains('list_desa_nama', $lp->nama)) {
                        $keterangan = 'Memiliki Akun';
                    }
                    echo $keterangan;
                @endphp</td>
                <td>{{ $lp->alamat_kantor }}</td>
            </tr>
            @php
                $no++
            @endphp
    ...
    <table border="2px" style="width: 100%;
        ...
        <tbody>
            @php
                $no=1;
            @endphp
            @foreach ($dataJadwal as $lp)
            <tr>
                <th>{{ $no }}</th>
                <th>{{ $lp->akun_desa_id }}</th>
                <th>{{ $lp->tgl_pemilihan }}</th>
                <th>{{ $lp->waktu_mulai }}</th>
                <th>{{ $lp->waktu_selesai }}</th>
            </tr>
            @php
                $no++
            @endphp
