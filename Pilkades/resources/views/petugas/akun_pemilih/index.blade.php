...
<h1 class="mt-4">Akun Pemilih</h1>
<ol class="breadcrumb mb-4">
    <li class="breadcrumb-item"><a href={{route('petugas.dashboard')}}>Dashboard</a></li>
    <li class="breadcrumb-item active">Akun Pemilih</li>
</ol>
@if(session('error'))
...
<div class="card mb-4">
    <div class="card-header d-flex justify-content-between items-center">
        <div class="pt-2">
            ...
            <div class="text-end">
                <a class="btn btn-primary" href="{{route('akun_pemilih.create')}}">
                    Tambah Data
                </a>
                <a class="btn btn-success" href="{{route('exportakunpemilih')}}">
            ...
            <div class="modal-dialog" role="document">
                ...
                <div class="modal-body">
                    ...
                    <form action="{{ route('importakunpemilih') }}" method="POST" enctype="multipart/form-data">
                    @csrf
        ...
        <table id="datatablesSimple">
            ...
            <tbody>
                @php
                    $no=1;
                @endphp
                @foreach ($akun_pemilih as $ad)
                <tr>
                    <th>{{ $no }}</th>
                    <th>{{ $ad->list_pemilih->nik }}</th>
                    <th>{{ $ad->list_pemilih->nama }}</th>
                    <th>{{ $ad->password }}</th>
                    <th>{{ $ad->status }}</th>
                    <th>{{ $ad->akun_desa_id }}</th>
                    <td>
                        <a class="btn btn-warning btn-sm" href="{{ route('akun_pemilih.edit', $ad->id) }}">Edit</a>
                        ...
                        <div class="modal fade" id="hapusModal{{ $ad->id }}" tabindex="-1" aria-labelledby="hapusModalLabel{{ $ad->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Apakah Anda yakin ingin menghapus data Akun Pemilih ini?
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                        <form action="{{ route('akun_pemilih.destroy', $ad->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
